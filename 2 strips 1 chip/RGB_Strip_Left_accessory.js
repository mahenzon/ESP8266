...
var options = {
  port: 1883,
  host: MQTT_IP,
  clientId: 'RGB_Strip_1'  // name to be introduced for MQTT broker
};
...
var LightController = {
  name: "RGB Strip Left", //name of accessory
  ...
  updatePower: function() {
    if(this.lastPower != this.currentPower){
      ...
      if(this.currentPower){
        client.publish(lightTopic, 'pl1');
      }else{
        client.publish(lightTopic, 'pl0');
      }
...
// As we need to control RGB light and iOS sends color in HSB (HSV) format, we need to convert it.
      ...
      toPublish = 'cl' + formatNumberLength(red) + ',' + formatNumberLength(green) + ',' +  formatNumberLength(blue);
